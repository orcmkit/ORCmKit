function [h_l, h_v, P_l, P_v, flag_l_bd, flag_v_bd] = find_2P_boundaries_Fast(fluid, h_su, h_ex, P_su, P_ex, param)
...
if param.solub
    Tbubble_min_l = R245fa_POE_Tbubble(1-param.C_oil, P_l, CoolPropt_T_PQ(param.abs_lowLevel,param.CP_file, P_l, 0));
    h_l = (1-param.C_oil)*CoolPropt_H_QT(param.abs_lowLevel,param.CP_file, 0, Tbubble_min_l) + param.C_oil*PropsSI_ICP('H', 'T', Tbubble_min_l, 'P', P_l, param.fluid_lub);
else
    h_l = CoolPropt_H_PQ(param.abs_lowLevel,param.CP_file, P_l, 0);
end
end
...
if param.solub
    Tsat_pure_v = CoolPropt_T_PQ(param.abs_lowLevel,param.CP_file, P_v, 1); 
    Tbubble_min_v = R245fa_POE_Tbubble(1-param.C_oil, P_v, Tsat_pure_v);
    [~, h_v, ~, ~, ~, ~, ~, ~] = xP_solubMixt_Fast(param.C_oil, P_v, param.x_lim_vap, fluid, param.fluid_lub, Tbubble_min_v, Tsat_pure_v, param.fit_DTP_zeta, param);
else
    h_v = CoolPropt_H_PQ(param.abs_lowLevel,param.CP_file, P_v, 1);
end
end