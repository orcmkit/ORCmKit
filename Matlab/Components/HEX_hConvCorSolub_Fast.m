function out = HEX_hConvCorSolub_Fast(fluid_h, m_dot_h, P_h_su, in_h_su, fluid_c, m_dot_c, P_c_su, in_c_su, Q_dot, info, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c)
...
for j = 1:length(out.H.T_vec)-1
    ...
    if strcmp(out.H.type_zone{j}, 'liq') || strcmp(out.H.type_zone{j}, 'vap') || strcmp(out.H.type_zone{j}, 'vap_wet')
        ...
        [mu_h, Pr_h, k_h, mu_rat_h] = PropsSinglePhase_Fast(T_h_mean, P_h_mean, T_wall_h, T_h_sat_mean, out.H.type_zone{j}, fluid_h, info.H);
    ...
    if strcmp(out.H.type_zone{j}, 'tp') || strcmp(out.H.type_zone{j}, 'vap_wet')
        ...
        [cp_h_l, k_h_l, mu_h_l, rho_h_l] = CoolPropt_Props_PQ(info.H.abs_lowLevel,info.H.CP_file, (0.5*out.H.P_vec(j)+0.5*out.H.P_vec(j+1)), 0);
        Pr_h_l = cp_h_l*mu_h_l/k_h_l;
        [~, ~, mu_h_v, rho_h_v] = CoolPropt_Props_PQ(info.H.abs_lowLevel,info.H.CP_file, (0.5*out.H.P_vec(j)+0.5*out.H.P_vec(j+1)), 1);
        i_fg_h = CoolPropt_H_PQ(info.H.abs_lowLevel,info.H.CP_file, (0.5*out.H.P_vec(j)+0.5*out.H.P_vec(j+1)), 1) - CoolPropt_H_PQ(info.H.abs_lowLevel,info.H.CP_file, (0.5*out.H.P_vec(j)+0.5*out.H.P_vec(j+1)), 0);
    ...
    if strcmp(out.C.type_zone{j}, 'liq') || strcmp(out.C.type_zone{j}, 'vap') || strcmp(out.C.type_zone{j}, 'tp_dryout')
        ...
        [mu_c, Pr_c, k_c, mu_rat_c] = PropsSinglePhase_Fast(T_c_mean, P_c_mean, T_wall_c, T_c_sat_mean, out.C.type_zone{j}, fluid_c, info.C);
    ...
    if strcmp(out.C.type_zone{j}, 'tp') || strcmp(out.C.type_zone{j}, 'tp_dryout')
        ...
        [cp_c_l, k_c_l, mu_c_l, rho_c_l] = CoolPropt_Props_PQ(info.C.abs_lowLevel,info.C.CP_file, (0.5*out.C.P_vec(j)+0.5*out.C.P_vec(j+1)), 0);
        Pr_c_l = cp_c_l*mu_c_l/k_c_l;
        [~, ~, mu_c_v, rho_c_v] = CoolPropt_Props_PQ(info.C.abs_lowLevel,info.C.CP_file, (0.5*out.C.P_vec(j)+0.5*out.C.P_vec(j+1)), 1);
        i_fg_c = CoolPropt_H_PQ(info.C.abs_lowLevel,info.C.CP_file, (0.5*out.C.P_vec(j)+0.5*out.C.P_vec(j+1)), 1) - CoolPropt_H_PQ(info.C.abs_lowLevel,info.C.CP_file, (0.5*out.C.P_vec(j)+0.5*out.C.P_vec(j+1)), 0);
    ...
    if not(dry_out_c) && strcmp(out.C.type_zone{j}, 'tp') %only compute if dry out has not yet started
        switch info.C.correlation.dry_out_incipient
            ...
            case 'Kim_DOI'
                P_star_c = (0.5*out.C.P_vec(j)+0.5*out.C.P_vec(j+1))/info.C.P_crit; 
                q_c = out.Qdot_vec(j)/out.C.A_vec(j);
                sigma_c_l = CoolPropt_I_PQ(info.C.abs_lowLevel,info.C.CP_file, (0.5*out.C.P_vec(j)+0.5*out.C.P_vec(j+1)), 0) ; %CoolProp.PropsSI('I', 'Q', 0, 'P', (0.5*out.C.P_vec(j)+0.5*out.C.P_vec(j+1)), fluid_c);
