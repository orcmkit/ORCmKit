function [out,TS] = ExpanderModel5_Solub(fluid, fluid_lub, C_oil, P_su, h_mix_su, N_exp, P_ex, T_amb, param)
        ...
        case 'SemiEmp'
            ...
            while not(stop) && j <= length(x_T_guess)
                ...
                while not(stop) && k <= length(ff_guess)
                    ...
                    [x, res, flag] = fsolve(@(x)  Exp_SemiEmp_res(x, ub, fluid, P_su, h_req_su, N_exp, param.V_s, param.r_v_in, P_ex, A_leak, param.d_su, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb, T_amb, param.C_loss, param.h_min, param.h_max, param), x0./ub, options);
            ...
            x = x.*ub;
            int = Exp_SemiEmp(x(1), x(2), fluid, P_su, h_req_su, N_exp, param.V_s, param.r_v_in, P_ex, A_leak, param.d_su, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb, T_amb, param.C_loss, param.h_min, param.h_max, param);
            M_dot_r = x(1);
            ...
            int.W_dot_oil = M_dot_oil*( (P_su-P_ex)/rho_oil_su) ;%(h_oil_su - h_oil_ex) +
...
if out.flag > 0;
    ...
    out.T_w = NaN;
    out.M_dot = N_exp/60*param.V_s*rho_req_su;
    out.FF = 1;
