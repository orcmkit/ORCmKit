% function [out_ORC, TS_ORC] = OrcModel4(fluid_wf, fluid_htf, in_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, P_ctf_su, T_amb, N_exp, P_cd, DT_sh, param)
%% CODE DESCRIPTION
...
%     m_dot_ctf = 1.4136;
    T_amb = 288.15;
    N_exp = 4735;
    in_ctf_su = CoolProp.PropsSI('H','P',P_ctf_su,'T',T_amb,'R245fa');
    P_cd = CoolProp.PropsSI('H','Q',0,'T',T_amb+10,fluid_wf);
    DT_sh = 10;
    param.solverType = 'M_imposed';
    param.DT_sc = 9;
    param.M_tot = 25.8746388;
    
    h_min =  CoolProp.PropsSI('H','P',5e4,'T',253.15,'R245fa');
    h_max =  CoolProp.PropsSI('H','P',4e6,'T',250+273.15,'R245fa');    
    ...
    param.EV.A_h_tot = param.EV.A_tot;
    param.EV.A_c_tot = param.EV.A_tot;
    ...
    param.REC.A_h_tot = param.REC.A_tot;
    param.REC.A_c_tot = param.REC.A_tot;
    ...
    param.PP.h_min = h_min;
    param.PP.h_max = h_max;
    
    EXP_folder = [path '\Expander\'];
    load([EXP_folder, 'ParametersCalibration_EXP.mat'])
    load([EXP_folder, 'PolReg_expS2P.mat'])
    ...
    param.EXP.h_min = h_min;
    param.EXP.h_max = h_max;
    param.EXP.PolReg = PolReg;
    ...
    param.LossesHP.h_min = h_min;
    param.LossesHP.h_max = h_max;
    
    param.LossesLP = DPLP_PhiDP;
    param.LossesLP.displayResults = 0;
    param.LossesLP.displayTS = 0;
    param.LossesLP.h_min = h_min;
    param.LossesLP.h_max = h_max;
...
% lb = [P_cd*1.01  CoolProp.PropsSI('H', 'P', P_cd, 'T', T_amb+0.1, fluid_wf)];
% ub = [CoolProp.PropsSI('P', 'Q', 0, 'T', min(CoolProp.PropsSI('Tcrit', 'Q', 0, 'T',273, fluid_wf)-2, T_htf_su-DT_sh), fluid_wf)  CoolProp.PropsSI('H', 'P', P_cd, 'Q', 0, fluid_wf)];
% x0 = (0.4*lb+0.4*ub)./ub;%[3.7309e+05 2.2005e+05]; 
...
% 
%ORC = FCT_ORC_Pcd_DTsh_fast( x, lb, ub, fluid_wf, fluid_htf, in_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, P_ctf_su, T_amb, N_exp, P_cd, DT_sh, param);
%   
...
P_exp_su_lb = P_cd*1.005;
P_exp_su_ub = CoolProp.PropsSI('P', 'Q', 0, 'T', min(CoolProp.PropsSI('Tcrit', 'Q', 0, 'T',273, fluid_wf)-2, T_htf_su-DT_sh), fluid_wf);
h_pp_su_lb = CoolProp.PropsSI('H', 'P', P_cd, 'T', T_amb+0.1, fluid_wf);
h_pp_su_ub = CoolProp.PropsSI('H', 'P', P_cd, 'Q', 0.5, fluid_wf);
ub = [P_exp_su_ub h_pp_su_ub];
lb = [P_exp_su_lb h_pp_su_lb];
P_exp_su_vec_x0 = linspace(P_exp_su_lb*1.1,P_exp_su_ub*0.8,param.init(1));
h_pp_su_vec_x0 = linspace(h_pp_su_lb*1.1,h_pp_su_ub*0.8,param.init(2));
[P_exp_su_guess_vec, h_pp_su_guess_vec, res] = deal(NaN*ones(1,length(P_exp_su_vec_x0)*length(h_pp_su_vec_x0)));
index = 0;
for i_pexp_su = 1: length(P_exp_su_vec_x0)
    for i_hpp_su = 1: length(h_pp_su_vec_x0)
        index = index+1;
        if param.display
            dispstat(['x0 evaluation: ' num2str(index) '/' num2str(length(P_exp_su_vec_x0)*length(h_pp_su_vec_x0))])
        end
        P_exp_su_guess_vec(index) = P_exp_su_vec_x0(i_pexp_su);
        h_pp_su_guess_vec(index) = h_pp_su_vec_x0(i_hpp_su);
        guess = FCT_ORC_Pcd_DTsh_fast([P_exp_su_guess_vec(index) h_pp_su_guess_vec(index)]./ub, lb, ub, fluid_wf, fluid_htf, in_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, P_ctf_su, T_amb, N_exp, P_cd, DT_sh, param);
...
%     res = [res_x0 , res];
%     if max(param.init) <2
%         if length(res(not(isnan(res)))) < 1 || min(res(not(isnan(res)))) > 1
%             out_ORC.flag_ORC = - 1;
%             TS_ORC = NaN;
%             return
%         end
...
%         
%         if strcmp(param.solverType, 'DTsc_imposed')
            x0 = [P_exp_su_guess_vec(j_order(k)) h_pp_su_guess_vec(j_order(k))]; 
...
f = @(x) FCT_ORC_Pcd_DTsh_res(x, lb, ub, fluid_wf, fluid_htf, in_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, P_ctf_su, T_amb, N_exp, P_cd, DT_sh, param);
...
x = fmincon(f,x0./ub,[],[],[],[],lb./ub,ub./ub,[],options_fmincon);
[out_ORC TS_ORC] = FCT_ORC_Pcd_DTsh( x, lb, ub, fluid_wf, fluid_htf, in_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, P_ctf_su, T_amb, N_exp, P_cd, DT_sh, param);
...
TS_ORC.cycle.s = TS_ORC.PP.s;
TS_ORC.cycle.T = TS_ORC.PP.T;
if isfield(param, 'REC')
    TS_ORC.cycle.s = [TS_ORC.cycle.s  TS_ORC.REC.s_c];
    TS_ORC.cycle.T = [TS_ORC.cycle.T  TS_ORC.REC.T_c];
end
if isfield(param, 'PRE')
    TS_ORC.cycle.s = [TS_ORC.cycle.s  TS_ORC.PRE.s_c];
    TS_ORC.cycle.T = [TS_ORC.cycle.T  TS_ORC.PRE.T_c];
end
TS_ORC.cycle.s = [TS_ORC.cycle.s  TS_ORC.EV.s_c];
TS_ORC.cycle.T = [TS_ORC.cycle.T  TS_ORC.EV.T_c];
if isfield(param, 'LossesHP')
    TS_ORC.cycle.s = [TS_ORC.cycle.s TS_ORC.LossesHP.s];
    TS_ORC.cycle.T = [TS_ORC.cycle.T TS_ORC.LossesHP.T];
end
TS_ORC.cycle.s = [TS_ORC.cycle.s  TS_ORC.EXP.s];
TS_ORC.cycle.T = [TS_ORC.cycle.T  TS_ORC.EXP.T];
if isfield(param, 'REC')
    TS_ORC.cycle.s = [TS_ORC.cycle.s  fliplr(TS_ORC.REC.s_h)];
    TS_ORC.cycle.T = [TS_ORC.cycle.T  fliplr(TS_ORC.REC.T_h)];
end
TS_ORC.cycle.s = [TS_ORC.cycle.s fliplr(TS_ORC.CD.s_h)];
TS_ORC.cycle.T = [TS_ORC.cycle.T fliplr(TS_ORC.CD.T_h)];
if isfield(param, 'SUB')
    TS_ORC.cycle.s = [TS_ORC.cycle.s  fliplr(TS_ORC.SUB.s_h)];
    TS_ORC.cycle.T = [TS_ORC.cycle.T  fliplr(TS_ORC.SUB.T_h)];
end
if isfield(param, 'LossesLP')
    TS_ORC.cycle.s = [TS_ORC.cycle.s TS_ORC.LossesLP.s];
    TS_ORC.cycle.T = [TS_ORC.cycle.T TS_ORC.LossesLP.T];
end
...
% end
% 
% function res = OrganicRankineCycle_res( x, lb, ub, fluid_wf, fluid_htf, in_htf_su, T_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, P_ctf_su, T_amb, N_exp, P_cd, DT_sh, param)
% [out, ~] = OrganicRankineCycle_Fast( x, lb, ub, fluid_wf, fluid_htf, in_htf_su, T_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, P_ctf_su, T_amb, N_exp, P_cd, DT_sh, param);
% res = out.res;
% end
% 
...
% [out_EXP, TS_EXP] = ExpanderModel3(fluid_wf, P_prev, h_prev, N_exp, P_exp_ex, T_amb, param.EXP);
...
% % out.N_exp_bis = out_EXP.N_exp;
% [out_EXP, ~] = ExpanderModel3(fluid_wf, out.P_exp_su, out.h_exp_su, N_exp, P_exp_ex, T_amb, param.EXP);
% out.m_dot_wf_bis = out_EXP.M_dot;
% out.W_dot_exp = out_EXP.W_dot;
% out.Q_dot_exp = out_EXP.Q_dot_amb;
% out.M_exp = out_EXP.M;
