function [out_ORC, TS_ORC] = OrcModel(fluid_wf, fluid_htf, in_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, P_ctf_su, m_dot_ctf, T_amb, N_exp, N_pp, ORC)
%% DEMONSTRATION CASE
if nargin == 0
    clear all
    close all
...
P_cd_lb = CoolProp.PropsSI('P', 'Q', 0, 'T', T_ctf_su + ORC.DT_sc, fluid_wf);
...
P_cd_guess0 = [CoolProp.PropsSI('P', 'Q', 0, 'T', T_ctf_su + ORC.DT_sc + 5, fluid_wf)  CoolProp.PropsSI('P', 'Q', 0, 'T', T_ctf_su + ORC.DT_sc + 15, fluid_wf) CoolProp.PropsSI('P', 'Q', 0, 'T', T_ctf_su + ORC.DT_sc + 25, fluid_wf)  CoolProp.PropsSI('P', 'Q', 0, 'T', T_ctf_su + ORC.DT_sc + 35, fluid_wf)];
x_rp_guess0 = [0.1 0.25 0.4 0.55];
x_Q_dot_rec_guess0 = [0 0.01 0.1 0.3 0.6 0.8];
x_Q_dot_pre_guess0 = [0 0.01 0.1 0.3 0.6 0.8];
x_Q_dot_cd_guess0 = [0 0.01 0.1 0.3 0.6 0.8];
index = 0;

[res,P_cd_guess_vec, P_cd_lb_vec, P_cd_ub_vec, P_ev_guess_vec, P_ev_lb_vec, P_ev_ub_vec, Q_dot_rec_guess_vec, Q_dot_rec_lb_vec, Q_dot_rec_ub_vec, Q_dot_pre_guess_vec, Q_dot_pre_lb_vec, Q_dot_pre_ub_vec,  Q_dot_cd_guess_vec, Q_dot_cd_lb_vec, Q_dot_cd_ub_vec] = deal(NaN*ones(1,length(P_cd_guess0)*length(x_rp_guess0)*length(x_Q_dot_rec_guess0)*length(x_Q_dot_pre_guess0)*length(x_Q_dot_cd_guess0)));
for i_pcd = 1: length(P_cd_guess0)
   for i_rp = 1: length(x_rp_guess0)
       for i_qdot_rec = 1: length(x_Q_dot_rec_guess0)
           for i_qdot_pre = 1: length(x_Q_dot_pre_guess0)
               for i_qdot_cd = 1: length(x_Q_dot_cd_guess0)
                   index = index+1;
                   dispstat(['Definition of the initial conditions  x0 : ' num2str(index) '/' num2str(length(P_cd_guess_vec))])
                   P_cd_guess_vec(index) = P_cd_guess0(i_pcd);
                   P_cd_lb_vec(index) = P_cd_lb;
                   P_cd_ub_vec(index) = P_cd_ub;
                   
                   P_ev_guess_vec(index) = min(P_cd_guess_vec(index)*(x_rp_guess0(i_rp)*rp_max + (1-x_rp_guess0(i_rp))*rp_min),P_ev_ub);
                   P_ev_lb_vec(index) = P_ev_lb;
                   P_ev_ub_vec(index) = P_ev_ub;
                   
                   z = [x_Q_dot_rec_guess0(i_qdot_rec), x_Q_dot_pre_guess0(i_qdot_pre), x_Q_dot_cd_guess0(i_qdot_cd)];
                   %if index == 25
                   ...
                   Q_dot_rec_lb_vec(index) = 0;
                   Q_dot_rec_ub_vec(index) = guess.Q_dot_rec_max;
                   Q_dot_rec_guess_vec(index) = guess.Q_dot_rec_guess;
                   
                   Q_dot_pre_lb_vec(index) = 0;
                   Q_dot_pre_ub_vec(index) = guess.Q_dot_pre_max;
                   Q_dot_pre_guess_vec(index) = guess.Q_dot_pre_guess;
                   
                   Q_dot_cd_lb_vec(index) = 0;
                   Q_dot_cd_ub_vec(index) = guess.Q_dot_cd_max;
                   Q_dot_cd_guess_vec(index) = guess.Q_dot_cd_guess;
                   
                   if guess.flag_pp > 0 && guess.flag_exp > 0 && guess.flag_rec > 0 && guess.flag_pre > 0 && guess.flag_ev > 0 && guess.flag_cd > 0 && guess.flag_sub > 0
                       res(index) = guess.res;
                   else
                       res(index) = NaN;
                   end
...
if isfield(ORC, 'x0') 
    ...
    P_cd_fit = lsqcurvefit(f5lin,[1 1 1 1 1 1],[ORC.x0.m_dot_ctf', ORC.x0.h_ctf_su'./1e5, ORC.x0.m_dot_htf',ORC.x0.h_htf_su'./1e5, ORC.x0.N_pp'./100],ORC.x0.P_pp_su'./1e5, [], [], options);
    P_cd_guess = f5lin(P_cd_fit, [m_dot_ctf, in_ctf_su/1e5, m_dot_htf, in_htf_su/1e5, N_pp/100])*1e5;
    
    P_ev_fit = lsqcurvefit(f5lin,[1 1 1 1 1 1],[ORC.x0.m_dot_ctf', ORC.x0.h_ctf_su'./1e5, ORC.x0.m_dot_htf',ORC.x0.h_htf_su'./1e5, ORC.x0.N_pp'./100],ORC.x0.P_recc_su'./1e5, [], [], options);
    P_ev_guess = f5lin(P_ev_fit, [m_dot_ctf, in_ctf_su/1e5, m_dot_htf, in_htf_su/1e5, N_pp/100])*1e5;
    
    Q_dot_rec_fit = lsqcurvefit(f5lin,[1 1 1 1 1 1],[ORC.x0.m_dot_ctf', ORC.x0.h_ctf_su'./1e5, ORC.x0.m_dot_htf',ORC.x0.h_htf_su'./1e5, ORC.x0.N_pp'./100],ORC.x0.Q_dot_rec'./1e3, [], [], options);
    Q_dot_rec_guess = f5lin(Q_dot_rec_fit, [m_dot_ctf, in_ctf_su/1e5, m_dot_htf, in_htf_su/1e5, N_pp/100])*1e3;
    
    Q_dot_pre_fit = lsqcurvefit(f5lin,[1 1 1 1 1 1],[ORC.x0.m_dot_ctf', ORC.x0.h_ctf_su'./1e5, ORC.x0.m_dot_htf',ORC.x0.h_htf_su'./1e5, ORC.x0.N_pp'./100],ORC.x0.Q_dot_pre'./1e3, [], [], options);
    Q_dot_pre_guess = f5lin(Q_dot_pre_fit, [m_dot_ctf, in_ctf_su/1e5, m_dot_htf, in_htf_su/1e5, N_pp/100])*1e3;
    
    Q_dot_cd_fit = lsqcurvefit(f5lin,[1 1 1 1 1 1],[ORC.x0.m_dot_ctf', ORC.x0.h_ctf_su'./1e5, ORC.x0.m_dot_htf',ORC.x0.h_htf_su'./1e5, ORC.x0.N_pp'./100],ORC.x0.Q_dot_cd'./1e3, [], [], options);
    Q_dot_cd_guess = f5lin(Q_dot_cd_fit, [m_dot_ctf, in_ctf_su/1e5, m_dot_htf, in_htf_su/1e5, N_pp/100])*1e3;

    x0_vec = [1  0.8  1.2 0.6 1.4 ];
    ...
    Q_dot_rec_lb_vec_x0 = 0*ones(1,length(x0_vec));
    Q_dot_rec_ub_vec_x0 = max(ORC.x0.Q_dot_rec)*ones(1,length(x0_vec));
    
    Q_dot_pre_guess_vec_x0 = Q_dot_pre_guess*x0_vec;
    Q_dot_pre_lb_vec_x0 = 0*ones(1,length(x0_vec));
    Q_dot_pre_ub_vec_x0 = max(ORC.x0.Q_dot_pre)*ones(1,length(x0_vec));
    
    Q_dot_cd_guess_vec_x0 = Q_dot_cd_guess*x0_vec;   
    Q_dot_cd_lb_vec_x0 = 0*ones(1,length(x0_vec));
    Q_dot_cd_ub_vec_x0 = max(ORC.x0.Q_dot_cd)*ones(1,length(x0_vec));
    
    res_x0 = NaN*ones(1,length(x0_vec));
    for k = 1:length(x0_vec)
        index = index+1;
        dispstat(['Definition of the initial conditions  x0 : ' num2str(index) '/' num2str(length(P_cd_guess_vec)+length(x0_vec))])
        out_x0 = OrganicRankineCycle([P_ev_guess_vec_x0(k) P_cd_guess_vec_x0(k) Q_dot_rec_guess_vec_x0(k) Q_dot_pre_guess_vec_x0(k) Q_dot_cd_guess_vec_x0(k)]./[P_ev_ub_vec_x0(k) P_cd_ub_vec_x0(k) Q_dot_rec_ub_vec_x0(k) Q_dot_pre_ub_vec_x0(k) Q_dot_cd_ub_vec_x0(k)], [P_ev_lb_vec_x0(k) P_cd_lb_vec_x0(k) Q_dot_rec_lb_vec_x0(k) Q_dot_pre_lb_vec_x0(k) Q_dot_cd_lb_vec_x0(k)], [P_ev_ub_vec_x0(k) P_cd_ub_vec_x0(k) Q_dot_rec_ub_vec_x0(k) Q_dot_pre_ub_vec_x0(k) Q_dot_cd_ub_vec_x0(k)], fluid_wf, fluid_htf, in_htf_su, T_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, T_ctf_su, P_ctf_su, m_dot_ctf, T_amb, N_exp, N_pp, ORC);
        if out_x0.flag_pp > 0 && out_x0.flag_exp > 0 && out_x0.flag_rec > 0 && out_x0.flag_pre > 0 && out_x0.flag_ev > 0 && out_x0.flag_cd > 0 && out_x0.flag_sub > 0
            res_x0(k) = out_x0.res;
        else
            res_x0(k) = NaN;
        end
    ...
    Q_dot_pre_guess_vec = [Q_dot_pre_guess_vec_x0 Q_dot_pre_guess_vec];
    Q_dot_pre_lb_vec = [Q_dot_pre_lb_vec_x0 Q_dot_pre_lb_vec];
    Q_dot_pre_ub_vec = [Q_dot_pre_ub_vec_x0 Q_dot_pre_ub_vec];
    
    Q_dot_cd_guess_vec = [Q_dot_cd_guess_vec_x0 Q_dot_cd_guess_vec];
    Q_dot_cd_lb_vec = [Q_dot_cd_lb_vec_x0 Q_dot_cd_lb_vec];
    Q_dot_cd_ub_vec = [Q_dot_cd_ub_vec_x0 Q_dot_cd_ub_vec];
...
Q_dot_pre_guess_vec = Q_dot_pre_guess_vec(not(isnan(res)));
Q_dot_pre_lb_vec = Q_dot_pre_lb_vec(not(isnan(res)));
Q_dot_pre_ub_vec = Q_dot_pre_ub_vec(not(isnan(res)));

Q_dot_cd_guess_vec = Q_dot_cd_guess_vec(not(isnan(res)));
Q_dot_cd_lb_vec = Q_dot_cd_lb_vec(not(isnan(res)));
Q_dot_cd_ub_vec = Q_dot_cd_ub_vec(not(isnan(res)));
...
dispstat('','init')
disp(['res x0:' num2str(res_ordered(1:min(Nbr_comb_x0,50)))])

k= 1;
stop = 0;
options = optimset('Display','none','TolX', 1e-8, 'TolFun', 1e-8);
dispstat('','init')
while not(stop) && k <= min(Nbr_comb_x0,70);
    dispstat(['Iterations starting for x0:' num2str(k) '/' num2str(min(Nbr_comb_x0,50)) ' - Res x0 = ' num2str(res(j_order(k)))])
    x0 = [P_ev_guess_vec(j_order(k))    P_cd_guess_vec(j_order(k))  Q_dot_rec_guess_vec(j_order(k))     Q_dot_pre_guess_vec(j_order(k))     Q_dot_cd_guess_vec(j_order(k))];
    ub = [P_ev_ub_vec(j_order(k))       P_cd_ub_vec(j_order(k))     Q_dot_rec_ub_vec(j_order(k))    	Q_dot_pre_ub_vec(j_order(k))	    Q_dot_cd_ub_vec(j_order(k))];
    lb = [P_ev_lb_vec(j_order(k))       P_cd_lb_vec(j_order(k))     Q_dot_rec_lb_vec(j_order(k))        Q_dot_pre_lb_vec(j_order(k))        Q_dot_cd_lb_vec(j_order(k))];
    ...
    out_ORC.funcCount = output.funcCount;
    out_ORC.iterations = output.iterations;

    if (abs(out_ORC.res_ORC_Hsu) < 1e-3) && (abs(out_ORC.res_ORC_Mdot) < 1e-3) && (abs(out_ORC.res_ORC_Qdot_rec) < 1e-3) && (abs(out_ORC.res_ORC_Qdot_pre) < 1e-3) && (abs(out_ORC.res_ORC_Qdot_cd) < 1e-3) && (out_ORC.flag_pp>0)  && (out_ORC.flag_exp>0)  && (out_ORC.flag_ev>0) && (out_ORC.flag_pre>0)  && (out_ORC.flag_rec>0)  && (out_ORC.flag_cd >0) && (out_ORC.flag_sub >0)
        out_ORC.flag_ORC = 1;
        stop = 1;
    else
        out_ORC.flag_ORC = - 1;
    ...
    dispstat(['Iterations starting for x0:' num2str(k) '/' num2str(min(Nbr_comb_x0,50)) ' - Res x0 = ' num2str(res(j_order(k))) ' - Res iter = ' num2str(out_ORC.res) ' flag ORC = ' num2str(out_ORC.flag_ORC) ' - Res H_su = ' num2str(out_ORC.res_ORC_Hsu) ' - Res Mdot = ' num2str(out_ORC.res_ORC_Mdot) ' - Res Qdot_rec = ' num2str(out_ORC.res_ORC_Qdot_rec) ' - Res Qdot_pre = ' num2str(out_ORC.res_ORC_Qdot_pre) ' - Res Qdot_cd = '  num2str(out_ORC.res_ORC_Qdot_cd)], 'keepthis')
...
%     P_prev = out.P_ev_su;
    ...
    out.T_pre_su = T_prev;
    out.h_pre_su = h_prev;    
    out.h_ev_su = min(out.h_pre_su + x(4)/out.m_dot_wf, CoolProp.PropsSI('H', 'P', out.P_pre_su, 'T', T_htf_su, fluid_wf));
    out.P_ev_su = out.P_pre_su;
    ...
    [out_PRE, TS_PRE] = HexModel(fluid_htf, P_htf_su, out.h_htf_ev_ex, m_dot_htf, fluid_wf, out.P_pre_su, out.h_pre_su, out.m_dot_wf , ORC.PRE);
    out.flag_pre = out_PRE.flag; 
    out.time_pre = out_PRE.time;
    out.Q_dot_pre = out_PRE.Q_dot_tot; 
    out.h_htf_pre_ex = out_PRE.h_h_ex;
    out.T_htf_pre_ex = out_PRE.T_h_ex;
    out.pinch_pre = out_EV.pinch;
...
%     P_prev = out.P_sub_su;
    ...
    out.T_cd_su = T_prev;
    out.h_cd_su = h_prev;    
    out.h_sub_su = max(out.h_cd_su - x(5)/out.m_dot_wf, CoolProp.PropsSI('H', 'P', out.P_cd_su, 'T', T_ctf_su, fluid_wf));
    out.P_sub_su = out.P_cd_su;
    [out_SUB, TS_SUB] = HexModel(fluid_wf, out.P_sub_su, out.h_sub_su, out.m_dot_wf, fluid_ctf, P_ctf_su, in_ctf_su, m_dot_ctf, ORC.SUB);
    out.flag_sub = out_SUB.flag; 
    out.time_sub = out_SUB.time;
    out.Q_dot_sub = out_SUB.Q_dot_tot;      
    out.h_ctf_sub_ex = out_SUB.h_c_ex;
    out.T_ctf_sub_ex = out_SUB.T_c_ex;
    out.pinch_sub = out_SUB.pinch;
    [out_CD, TS_CD] = HexModel(fluid_wf, out.P_cd_su, out.h_cd_su, out.m_dot_wf, fluid_ctf, P_ctf_su, out.h_ctf_sub_ex, m_dot_ctf, ORC.CD);
    ...
    T_prev = out_SUB.T_h_ex;
    h_prev = out_SUB.h_h_ex;
    P_prev = out.P_sub_su;
...
if x(4) == 0 && out.Q_dot_pre == 0
    out.res_ORC_Qdot_pre = 0;
elseif x(4) == 0 && out.Q_dot_pre ~= 0
    out.res_ORC_Qdot_pre = 1;
elseif x(4) ~= 0 && out.Q_dot_pre == 0
    out.res_ORC_Qdot_pre = 1;
else
    out.res_ORC_Qdot_pre = 1 - x(4)/out.Q_dot_pre;
end

if x(5) == 0 && out.Q_dot_cd == 0
    out.res_ORC_Qdot_cd = 0;
elseif x(5) == 0 && out.Q_dot_cd ~= 0
    out.res_ORC_Qdot_cd = 1;
elseif x(5) ~= 0 && out.Q_dot_cd == 0
    out.res_ORC_Qdot_cd = 1;
else
    out.res_ORC_Qdot_cd = 1 - x(5)/out.Q_dot_cd;
end

if out.flag_pp < 0 || out.flag_exp < 0 || out.flag_rec < 0 || out.flag_pre < 0 || out.flag_ev < 0 || out.flag_cd < 0 || out.flag_sub < 0
    out.res  = 100*norm([out.res_ORC_Mdot    out.res_ORC_Hsu     out.res_ORC_Qdot_rec    out.res_ORC_Qdot_pre    out.res_ORC_Qdot_cd]);
else
    out.res = norm([out.res_ORC_Mdot    out.res_ORC_Hsu     out.res_ORC_Qdot_rec    out.res_ORC_Qdot_pre    out.res_ORC_Qdot_cd]);
end
out.x = x;
...
%     P_prev = out.P_ev_su;
    ...
    out.T_pre_su = T_prev;
    out.h_pre_su = h_prev; 
    out.Q_dot_pre_max = HEX_Qdotmax(fluid_htf, m_dot_htf, P_htf_su, in_htf_su, fluid_wf, out.m_dot_wf, out.P_pre_su, out.h_pre_su, ORC.PRE);
    out.Q_dot_pre_guess = z(2)*out.Q_dot_pre_max;
    out.h_ev_su = min(out.h_pre_su + out.Q_dot_pre_guess/out.m_dot_wf, CoolProp.PropsSI('H', 'P', out.P_pre_su, 'T', T_htf_su, fluid_wf));
    out.P_ev_su = out.P_pre_su;
    ...
    out.T_htf_ev_ex = out_EV.T_h_ex;
    out.pinch_ev = out_EV.pinch;
    [out_PRE, ~] = HexModel(fluid_htf, P_htf_su, out.h_htf_ev_ex, m_dot_htf, fluid_wf, out.P_pre_su, out.h_pre_su, out.m_dot_wf , ORC.PRE);
    out.flag_pre = out_PRE.flag; 
    out.time_pre = out_PRE.time;
    out.Q_dot_pre = out_PRE.Q_dot_tot; 
    out.h_htf_pre_ex = out_PRE.h_h_ex;
...
%     P_prev = out.P_sub_su;
    ...
    out.T_cd_su = T_prev;
    out.h_cd_su = h_prev;
    out.Q_dot_cd_max = HEX_Qdotmax(fluid_wf, out.m_dot_wf, out.P_cd_su, out.h_cd_su, fluid_ctf, m_dot_ctf, P_ctf_su, in_ctf_su, ORC.CD);
    out.Q_dot_cd_guess = z(3)*out.Q_dot_cd_max;
    out.h_sub_su = max(out.h_cd_su - out.Q_dot_cd_guess/out.m_dot_wf, CoolProp.PropsSI('H', 'P', out.P_cd_su, 'T', T_ctf_su, fluid_wf));
    out.P_sub_su = out.P_cd_su;
    [out_SUB, ~] = HexModel(fluid_wf, out.P_sub_su, out.h_sub_su, out.m_dot_wf, fluid_ctf, P_ctf_su, in_ctf_su, m_dot_ctf, ORC.SUB);
    out.flag_sub = out_SUB.flag; 
    out.time_sub = out_SUB.time;
    out.Q_dot_sub = out_SUB.Q_dot_tot;      
    out.h_ctf_sub_ex = out_SUB.h_c_ex;
    out.T_ctf_sub_ex = out_SUB.T_c_ex;
    out.pinch_sub = out_SUB.pinch;
    [out_CD, ~] = HexModel(fluid_wf, out.P_cd_su, out.h_cd_su, out.m_dot_wf, fluid_ctf, P_ctf_su, out.h_ctf_sub_ex, m_dot_ctf, ORC.CD);
    out.flag_cd = out_CD.flag; 
    out.time_cd = out_CD.time;
    out.Q_dot_cd = out_CD.Q_dot_tot; 
    out.T_ctf_cd_ex = out_CD.T_c_ex;
...
if out.Q_dot_pre_guess == 0 && out.Q_dot_pre == 0
    out.res_ORC_Qdot_pre = 0;
elseif out.Q_dot_pre_guess == 0 && out.Q_dot_pre ~= 0
    out.res_ORC_Qdot_pre = 1;
elseif out.Q_dot_pre_guess ~= 0 && out.Q_dot_pre == 0
    out.res_ORC_Qdot_pre = 1;
else
    out.res_ORC_Qdot_pre = 1 - out.Q_dot_pre_guess/out.Q_dot_pre;
end

if out.Q_dot_cd_guess == 0 && out.Q_dot_cd == 0
    out.res_ORC_Qdot_cd = 0;
elseif out.Q_dot_cd_guess == 0 && out.Q_dot_cd ~= 0
    out.res_ORC_Qdot_cd = 1;
elseif out.Q_dot_cd_guess ~= 0 && out.Q_dot_cd == 0
    out.res_ORC_Qdot_cd = 1;
else
    out.res_ORC_Qdot_cd = 1 - out.Q_dot_cd_guess/out.Q_dot_cd;
end
