function [out_ORC, TS_ORC] = OrcModel6(fluid_wf, fluid_htf, in_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, P_ctf_su, m_dot_ctf, T_amb, N_exp, N_pp, param)
%% CODE DESCRIPTION
...
elseif strcmp(param.solverType, 'M_imposed') %% CASE 2 : mass imposed
    
    param.solverType = 'DTsc_imposed';
    DT_sc_guess = 0;
    param.DT_sc = DT_sc_guess;
    ...
    Nbr_comb_x0 = length(j_order);
    Nbr_comb_x0_max = param.nbr_test;
    param.Nbr_comb =  min(Nbr_comb_x0,Nbr_comb_x0_max);
    ...
    IC.P_pp_ex_guess_vec = IC.P_pp_ex_guess_vec(j_order);
    IC.P_pp_su_guess_vec = IC.P_pp_su_guess_vec(j_order);
    IC.h_ev_ex_guess_vec = IC.h_ev_ex_guess_vec(j_order);
    IC.P_pp_ex_ub_vec = IC.P_pp_ex_ub_vec(j_order);
    IC.P_pp_su_ub_vec = IC.P_pp_su_ub_vec(j_order);
    IC.h_ev_ex_ub_vec = IC.h_ev_ex_ub_vec(j_order);
    IC.P_pp_ex_lb_vec = IC.P_pp_ex_lb_vec(j_order);
    IC.P_pp_su_lb_vec = IC.P_pp_su_lb_vec(j_order);
    IC.h_ev_ex_lb_vec = IC.h_ev_ex_lb_vec(j_order);
    IC.res = IC.res(j_order);
...
%     if param.display
        ...
        disp('Start iteration:')
        fprintf('\n');
        fprintf('%-10s %-10s %-50s %-15s %-50s %-60s %-15s %-10s %-100s\n', 'DT_sc', '#', 'x_in', 'res_in', 'x_out', 'res_out', 'res_M', 'flag_ORC', 'flag components');
        fprintf('\n');
%     end
    ...
    DT_lb = -1;
    DT_ub = 20;
    %options_fmincon = optimset('Disp',param.displayIter,'Algorithm','interior-point','UseParallel',false,'TolX',1e-6,'TolFun',1e-6,'TolCon',1e-6,'MaxIter',1e3,'OutputFcn',@outputfunFS_DT);
    ...
    f = @(x) FCT_ORC_guess_DT(x, DT_lb, DT_ub, fluid_wf, fluid_htf, in_htf_su, T_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, T_ctf_su, P_ctf_su, m_dot_ctf, T_amb, N_exp, N_pp, param);
    %[DT_ok,fval,exitflag,output] = fmincon(f,DT_0/DT_ub,[],[],[],[],DT_lb./DT_ub,DT_ub./DT_ub,[],options_fmincon)
    [DT_ok,fval,exitflag,output] = fsolve(f,DT_0/DT_ub,options_fsolve)
    ...
    [out_ORC, TS_ORC] = FCT_ORC_Ext_Npp_Nexp_2(x0/ub, lb, ub, fluid_wf, fluid_htf, in_htf_su, T_htf_su, P_htf_su, m_dot_htf, fluid_ctf, in_ctf_su, T_ctf_su, P_ctf_su, m_dot_ctf, T_amb, N_exp, N_pp, param);
    if any(out_ORC.flag.value<0)
        out_ORC.flag_ORC = - 1;
    elseif all(out_ORC.flag.value>0) && any(abs([out_ORC.res_vec out.res_ORC_M]) > 5e-5) 
        out_ORC.flag_ORC = -2;
    else
        out_ORC.flag_ORC = 1;
    end
...
function stop = outputfunFS_DT(x, optimValues, state)
%disp(norm(optimValues.fval))
stop = norm(optimValues.fval) < 5e-5;
end