function out = HEX_profile_Solub_DP2(fluid_h, m_dot_h, P_h_su, in_h_su, fluid_c, m_dot_c, P_c_su, in_c_su, Q_dot, param, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h_in, DP_c_in)
...
if not(param.H.DP_on) || DP_h_in <1e-2
    ...
    [h_h_l, h_h_v, ~, ~, ~, ~] = find_2P_boundaries(fluid_h, h_h_su, h_h_ex, P_h_su, P_h_ex, param.H);
end
...
if not(param.C.DP_on) || DP_c_in <1e-2
    ...
    [h_c_l, h_c_v, ~, ~, ~, ~] = find_2P_boundaries(fluid_c, h_c_su, h_c_ex, P_c_su, P_c_ex, param.C);    
end
...
for k = 1:length(out.C.H_vec)
    if param.C.solub
        ...
        if strcmp(fluid_c(1:3), 'ICP')
            ...
            out.C.Tsat_pure_vec(k) = CoolProp.PropsSI('T', 'Q', 0.5, 'P', out.C.P_vec(k), fluid_c);
            %h_c_l = CoolProp.PropsSI('H', 'Q', 0, 'P', out.C.P_vec(k), fluid_c);
            %h_c_v = CoolProp.PropsSI('H', 'Q', 1, 'P', out.C.P_vec(k), fluid_c);
            if out.C.H_vec(k)<h_c_l
                out.C.x_vec(k) = -2;
            elseif out.C.H_vec(k)>h_c_v
                out.C.x_vec(k) = 2;
            else
                out.C.x_vec(k) = min(1,max(0,(out.C.H_vec(k) - h_c_l)/(h_c_v - h_c_l))); %min(1,max(0,CoolProp.PropsSI('Q', 'H', out.C.H_vec(k), 'P', out.C.P_vec(k), fluid_c)));
            end
        end
    end
end
...
for k = 1:length(out.H.H_vec)
    if param.H.solub
        ...
        if strcmp(fluid_h(1:3), 'ICP')
            ...
            out.H.Tsat_pure_vec(k) = CoolProp.PropsSI('T', 'Q', 0.5, 'P', out.H.P_vec(k), fluid_h);
            %h_h_l = CoolProp.PropsSI('H', 'Q', 0, 'P', out.H.P_vec(k), fluid_h);
            %h_h_v = CoolProp.PropsSI('H', 'Q', 1, 'P', out.H.P_vec(k), fluid_h);
            if out.H.H_vec(k)<h_h_l
                out.H.x_vec(k) = -2;
            elseif out.H.H_vec(k)>h_h_v
                out.H.x_vec(k) = 2;
            else
                out.H.x_vec(k) = min(1,max(0,(out.H.H_vec(k) - h_h_l)/(h_h_v - h_h_l))); %min(1,max(0,CoolProp.PropsSI('Q', 'H', out.H.H_vec(k), 'P', out.H.P_vec(k), fluid_h)));
            end
        end
    end
end
