function [out,TS] = ExpanderModel3_Solub(fluid, fluid_lub, C_oil, P_su, h_mix_su, M_dot, P_ex, T_amb, param)
...
if nargin == 0
    ...
    P_su = 10e5;               %Supply pressure        [Pa]
    h_mix_su = CoolProp.PropsSI('H','P',P_su,'T',273.15+130,fluid);               %Supply enthalpy        [J/kg]
...
if C_oil >0
    % Split flow in oil and equivalent Ref
    Tsat_pure_su = CoolProp.PropsSI('T','P',P_su,'Q',0,fluid);
    Tbubble_min_su = R245fa_POE_Tbubble(1-C_oil, P_su, Tsat_pure_su);
    [T_su, h_rl_su, ~, h_rv_su, zeta_r_su, x_su, C_rl_su, C_rv_su] = HP_solubMixt(C_oil, P_su, h_mix_su, fluid, fluid_lub, Tbubble_min_su, Tsat_pure_su, []);
    h_req_su = (1-x_su)*h_rl_su + x_su*h_rv_su;
    M_dot_r = (1-C_oil)*M_dot;
    M_dot_oil = C_oil*M_dot;
    T_req_su = CoolProp.PropsSI('T','P',P_su,'H',h_req_su,fluid);
    s_req_su = CoolProp.PropsSI('S','P',P_su,'H',h_req_su,fluid);
    rho_req_su = CoolProp.PropsSI('D','P',P_su,'H',h_req_su,fluid);
    h_req_ex_s = CoolProp.PropsSI('H','P',P_ex,'S',s_req_su,fluid);
    %s_su = CoolProp.PropsSI('S','P',P_su,'T',T_su,fluid);
    T_is_ex = CoolProp.PropsSI('T','P',P_ex,'S',s_req_su,fluid);
    Tsat_ex = CoolProp.PropsSI('T', 'Q', 0, 'P', P_ex, fluid);
    Tbubble_min_ex = R245fa_POE_Tbubble(1-C_oil, P_ex, Tsat_ex);
    [h_ex_s, ~, ~, ~, ~, ~, ~, ~] = TP_solubMixt(T_is_ex, P_ex, C_oil, fluid, fluid_lub, Tbubble_min_ex, Tsat_ex, []);
    
    [rho_vap_su, ~, rho_oil_su, rho_liq_su, ~] = R245fa_POE_density(T_su, P_su, zeta_r_su, fluid, fluid_lub, param.fit_ratio_rho, Tbubble_min_su, Tsat_pure_su);
    alpha_su = VoidFraction_SlipRatio(C_rv_su, rho_vap_su, rho_liq_su, 5);
    V_swept = param.V_s*(alpha_su + (1-alpha_su)*(C_rl_su/(C_rl_su+C_oil)));
    ...
    T_su = CoolProp.PropsSI('T','P',P_su,'H',h_mix_su,fluid);
    T_req_su = T_su;
    
    rho_su = CoolProp.PropsSI('D','P',P_su,'H',h_mix_su,fluid);
    rho_req_su = rho_su;
    
    s_su = CoolProp.PropsSI('S','P',P_su,'H',h_mix_su,fluid);
    s_req_su = s_su;
    
    h_ex_s = CoolProp.PropsSI('H','P',P_ex,'S',s_su,fluid);
    h_req_ex_s = h_ex_s; 
    
    T_is_ex =CoolProp.PropsSI('T','P',P_ex,'H',h_ex_s,fluid);
    
    V_swept = param.V_s;
    
    C_rv_su = h_mix_su-CoolProp.PropsSI('H','P',P_su,'Q',0.1,fluid);
end


if P_su > P_ex && C_rv_su > 0%&& h_su > CoolProp.PropsSI('H','P',P_su,'Q',0.1,fluid);
    ...
    % Simulate WF process by means of semi-empirical model
    A_leak = param.A_leak0*(1-param.K_lk*C_oil);
    x0 = 0.85*T_req_su+0.15*T_amb; %initial value for T_wall
    ub = 2*x0;
    options = optimset('Display','off');
    ff = @(x)  FCT_Exp2_SemiEmp_res(x, ub, fluid, P_su, h_req_su, M_dot_r, V_swept, param.r_v_in, P_ex, A_leak, param.d_su, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb, T_amb, param.C_loss, param, s_req_su, rho_req_su, h_req_ex_s);
    [x, ~, flag] = fsolve(ff, x0./ub, options);
    x = x.*ub;
    T_w = x;
    int = FCT_Exp2_SemiEmp(x,fluid, P_su, h_req_su, M_dot_r, V_swept, param.r_v_in, P_ex, A_leak, param.d_su, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb, T_amb, param.C_loss, param, s_req_su, rho_req_su, h_req_ex_s);
    N_exp = int.N_exp;
    h_req_ex = int.h_ex;
    T_req_ex = CoolProp.PropsSI('T','P',P_ex,'H',h_req_ex,fluid);
    epsilon_is = int.epsilon_is;
    FF = M_dot_r/(V_swept*int.N_exp/60*rho_req_su);
    Q_dot_amb = int.Q_dot_amb;
    if  all(abs(int.res) < 1e-4) && h_req_ex > param.h_min && h_req_ex < param.h_max
        out.flag = flag;
    else
        out.flag = -1;
    end
    ...
    if C_oil>0
        % Simulate OIL process
        h_oil_ex = PropsSI_ICP('H', 'T', T_req_ex, 'P', P_ex, fluid_lub);
        W_dot_oil = M_dot_oil*((P_su-P_ex)/rho_oil_su) ;       
        W_dot = int.W_dot + W_dot_oil;      
        % Mixing exhaust
        h_ex = (M_dot_oil*h_oil_ex + M_dot_r*h_req_ex)/M_dot;
        Tsat_pure_ex = CoolProp.PropsSI('T','P',P_ex,'Q',0,fluid);
        Tbubble_min_ex = R245fa_POE_Tbubble(1-C_oil, P_ex, Tsat_pure_ex);
        [T_ex, ~, ~, ~, ~, ~, ~, ~] = HP_solubMixt(C_oil, P_ex, h_ex, fluid, fluid_lub, Tbubble_min_ex, Tsat_pure_ex, []);
    else
        W_dot = int.W_dot;
        h_ex = h_req_ex;
        T_ex = T_req_ex;
...
if out.flag > 0;
    ...
    out.T_su = T_su;
    out.T_w = NaN;
    out.N_exp = 60*M_dot/(V_swept*rho_req_su);
    out.FF = 1;
    out.W_dot = M_dot*(h_req_su-h_req_ex_s);
    out.epsilon_is =1;
    out.Q_dot_amb = 0;
    out.h_ex = h_ex_s;
    out.T_ex = T_is_ex;
...
if param.generateTS
    s_su = NaN;%CoolProp.PropsSI('S','H',h_su,'P',P_su, fluid);
    s_ex = NaN;%CoolProp.PropsSI('S','H', out.h_ex,'P', P_ex, fluid);
    TS.T = NaN;%[T_su out.T_ex];
    TS.s = NaN;%[s_su s_ex];
else
    TS.T = NaN;
    TS.s = NaN;
...
%% NESTED FUNCTIONS
function res = FCT_Exp2_SemiEmp_res(x, ub, fluid, P_su, h_su, M_dot, V_s, r_v_in, P_ex, A_leak, d_su, alpha, W_dot_loss_0, AU_su_n, M_dot_n, AU_ex_n, AU_amb, T_amb, C_loss, param, s_su, rho_su, h_ex_s)
T_w = x.*ub;
out = FCT_Exp2_SemiEmp(T_w, fluid, P_su, h_su, M_dot, V_s, r_v_in, P_ex, A_leak, d_su, alpha, W_dot_loss_0, AU_su_n, M_dot_n, AU_ex_n, AU_amb, T_amb, C_loss, param, s_su, rho_su, h_ex_s);
res = out.res;
end

function out = FCT_Exp2_SemiEmp(T_w, fluid, P_su, h_su, M_dot, V_s, r_v_in, P_ex, A_leak, d_su, alpha, W_dot_loss_0, AU_su_n, M_dot_n, AU_ex_n, AU_amb, T_amb, C_loss, param, s_su, rho_su, h_ex_s)
