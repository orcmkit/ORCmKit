function [out,TS] = HexModel_Solub_DP2_Fast(fluid_h, P_h_su, in_h_su, m_dot_h, fluid_c, P_c_su, in_c_su, m_dot_c, param)
...
[h_h_l_in, h_h_v_in, T_h_su] = supply_conditions_HEX_Solub_Fast(in_h_su, P_h_su, fluid_h, param.H);

% Evaluation of the cold fluid (CF) supply conditions
[h_c_l_in, h_c_v_in, T_c_su] = supply_conditions_HEX_Solub_Fast(in_c_su, P_c_su, fluid_c, param.C);
...
if (T_h_su-T_c_su)>1e-2  && m_dot_h  > 0 && m_dot_c > 0; % Check if the operating conditions permit a viable heat transfer
    ...
    %else
        [Q_dot_max, pinch_Qdot_max] = HEX_Qdotmax_Solub_DP2_Fast(fluid_h, m_dot_h, P_h_su, in_h_su, T_h_su, fluid_c, m_dot_c, P_c_su, in_c_su, T_c_su, param, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c); % compute maximal heat transfer
    %end
    lb = 0;
    ub = Q_dot_max;
    f = @(Q_dot) HEX_hConvCorSolub_res_Fast(fluid_h, m_dot_h, P_h_su, in_h_su, fluid_c, m_dot_c, P_c_su, in_c_su,  Q_dot, param, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c);
    %f(ub)
    %figure
...
out = HEX_hConvCorSolub_Fast(fluid_h, m_dot_h, P_h_su, in_h_su, fluid_c, m_dot_c, P_c_su, in_c_su, real(Q_dot_eff), param, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c);
out.H.fluid = fluid_h;
out.C.fluid = fluid_c;
...
[out.H.M_vec, out.H.M_mf_vec, out.H.M_lub_vec, out.H.alpha_mean_vec, out.H.M_tot, out.H.M_mf_tot, out.H.M_lub_tot] = generate_M_vec_Fast(param.H, out.H, fluid_h);
[out.C.M_vec, out.C.M_mf_vec, out.C.M_lub_vec, out.C.alpha_mean_vec, out.C.M_tot, out.C.M_mf_tot, out.C.M_lub_tot] = generate_M_vec_Fast(param.C, out.C, fluid_c);
