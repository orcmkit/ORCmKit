...
H_h_vec = H_h_vec_disc(not(H_h_vec_disc<h_h_ex | H_h_vec_disc>h_h_su));
...
H_c_vec = H_c_vec_disc(not(H_c_vec_disc>h_c_ex | H_c_vec_disc<h_c_su));
...
for k = 1:length(out.C.H_vec)
    if param.C.solub
        [out.C.T_vec(k), out.C.Tbubble_min_vec(k), out.C.Tsat_pure_vec(k), out.C.H_rl_vec(k), out.C.H_oil_vec(k), out.C.H_rv_vec(k), out.C.zeta_r_vec(k), out.C.x_vec(k), out.C.C_rl_vec(k), out.C.C_rv_vec(k)] = HP_solubMixt(param.C.C_oil, P_c_su, out.C.H_vec(k), fluid_c, param.C.fluid_lub);
    else
        if strcmp(fluid_c(1:3), 'ICP')
            out.C.T_vec(k) = PropsSI_ICP('T', 'H', out.C.H_vec(k), 'P', P_c_su, fluid_c);
        else
            out.C.T_vec(k) = CoolProp.PropsSI('T', 'H', out.C.H_vec(k), 'P', P_c_su, fluid_c);
            out.C.Tsat_pure_vec(k) = CoolProp.PropsSI('T', 'Q', 0.5, 'P', P_c_su, fluid_c);
        end
    end
end
out.H.H_vec = H_h_vec;
...
for k = 1:length(out.H.H_vec)
    if param.H.solub
        [out.H.T_vec(k), out.H.Tbubble_min_vec(k), out.H.Tsat_pure_vec(k), out.H.H_rl_vec(k), out.H.H_oil_vec(k), out.H.H_rv_vec(k), out.H.zeta_r_vec(k), out.H.x_vec(k), out.H.C_rl_vec(k), out.H.C_rv_vec(k)] = HP_solubMixt(param.H.C_oil, P_h_su, out.H.H_vec(k), fluid_h, param.H.fluid_lub);
    else
        if strcmp(fluid_h(1:3), 'ICP')
            out.H.T_vec(k) = PropsSI_ICP('T', 'H', out.H.H_vec(k), 'P', P_h_su, fluid_h);
        else
            out.H.T_vec(k) = CoolProp.PropsSI('T', 'H', out.H.H_vec(k), 'P', P_h_su, fluid_h);
            out.H.Tsat_pure_vec(k) = CoolProp.PropsSI('T', 'Q', 0.5, 'P', P_h_su, fluid_h);
        end
    end
end
