function out = CompressorNestedModel_3_vRD_3(fluid, P_su, T_su, N_rot, P_ex, T_amb, param)
...
while not(stop) && k_iter<min(length(x_T_guess),20)
   ...
   x0(3) = 0.9*param.V_s*N_rot/60*rho_su; %guess value on Mdot
    
   lb = [min(T_su,T_amb)-10     x0(2)-50    x0(3)/10];
   ub = [max(T_amb,T_ex_s)+50   x0(2)+50    x0(3)/0.9];
    
   options = optimoptions('fmincon','Display','none');
   [x, ~, flag] = fmincon(@(x)  FCT_Compr_SemiEmp_res(x, ub, fluid, N_rot, T_amb, P_su, T_su, h_su, P_ex, h_ex_s, param.V_s, param.r_v_in, param.A_leak0, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb,param.h_min,param.h_max),x0./ub, [],[],[],[],lb./ub, ub./ub, [], options);
   if flag > 0 
      stop = 1;
   end
end
x = x.*ub;
int = FCT_Compr_SemiEmp(x(1), x(2), x(3), fluid, N_rot, T_amb, P_su, T_su, h_su, P_ex, h_ex_s, param.V_s, param.r_v_in, param.A_leak0, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb,param.h_min,param.h_max);

out.M_dot = x(3);
out.N_rot = int.N_rot_bis;
out.W_dot = int.W_dot;
out.T_ex = CoolProp.PropsSI('T','H',int.h_ex,'P',P_ex,fluid);
out.VolEff = out.M_dot/(param.V_s*int.N_rot_bis/60*rho_su);
out.Q_dot_amb = int.Q_dot_amb;
out.epsilon_is = int.epsilon_is;
if int.resE < 1e-4 && int.res_hex2 < 1e-4  && int.res_Nrot < 1e-4  
    out.flag = flag;
else
    out.flag = -1;
end
...
function res = FCT_Compr_SemiEmp_res(x, ub, fluid, N_rot, T_amb, P_su, T_su, h_su, P_ex, h_ex_s, V_s, r_v_in, A_leak0, alpha, W_dot_loss_0, AU_su_n, M_dot_n, AU_ex_n, AU_amb,h_min,h_max)

x = x.*ub;
out = FCT_Compr_SemiEmp(x(1), x(2), x(3), fluid, N_rot, T_amb, P_su, T_su, h_su, P_ex, h_ex_s, V_s, r_v_in, A_leak0, alpha, W_dot_loss_0, AU_su_n, M_dot_n, AU_ex_n, AU_amb,h_min,h_max);
res = [out.resE; out.res_hex2; out.res_Nrot];
res = norm(res);
end

function out = FCT_Compr_SemiEmp(T_w, T_ex2, M_dot, fluid, N_rot, T_amb, P_su, T_su, h_su, P_ex, h_ex_s, V_s, r_v_in, A_leak0, alpha, W_dot_loss_0, AU_su_n, M_dot_n, AU_ex_n, AU_amb,~,~)
...
out.N_rot_bis = out.M_dot_in/V_s/out.rho_su2*60;
...
out.res_Nrot = abs(out.N_rot_bis - N_rot)/N_rot; 
end
