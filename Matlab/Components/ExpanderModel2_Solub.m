function [out,TS] = ExpanderModel2_Solub(fluid, fluid_lub, C_oil, P_su, h_mix_su, M_dot, P_ex, T_amb, param)
...
[T_su, h_rl_su, h_oil_su, h_rv_su, zeta_r_su, x_su, C_rl_su, C_rv_su] = HP_solubMixt(C_oil, P_su, h_mix_su, fluid, fluid_lub, Tbubble_min_su, Tsat_pure_su, []);
...
if P_su > P_ex %&& h_su > CoolProp.PropsSI('H','P',P_su,'Q',0.1,fluid);
    ...
    [x, ~, flag] = fsolve(@(x)  FCT_Exp2_SemiEmp_res(x, ub, fluid, P_su, h_req_su, M_dot_r, param.V_s, param.r_v_in, P_ex, A_leak, param.d_su, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb, T_amb, param.C_loss, param, s_req_su, rho_req_su, h_req_ex_s), x0./ub, options);
    x = x.*ub;
    T_w = x;
    int = FCT_Exp2_SemiEmp(x,fluid, P_su, h_req_su, M_dot_r, param.V_s, param.r_v_in, P_ex, A_leak, param.d_su, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb, T_amb, param.C_loss, param, s_req_su, rho_req_su, h_req_ex_s);
    N_exp = int.N_exp;
    h_req_ex = int.h_ex;
    T_req_ex = CoolProp.PropsSI('T','P',P_ex,'H',h_req_ex,fluid);
    epsilon_is = int.epsilon_is;
    FF = M_dot_r/(param.V_s*int.N_exp/60*rho_req_su);
    ...
    rho_oil_su = PropsSI_ICP('D', 'T', T_su, 'P', P_su, fluid_lub);
    h_oil_ex = PropsSI_ICP('H', 'T', T_req_ex, 'P', P_ex, fluid_lub);
    W_dot_oil = M_dot_oil*((h_oil_su - h_oil_ex) + (P_su-P_ex)/rho_oil_su) ;
...
if out.flag > 0;
    ...
    out.FF = FF;
    out.T_ex = T_ex; 
    out.M = (rho_req_su+CoolProp.PropsSI('D','H',h_req_ex,'P',P_ex,fluid))/2*param.V;   
    out.T_su = T_su;
else    
    out.T_su = T_su;
    out.T_w = NaN;
    out.N_exp = 60*M_dot/(param.V_s*rho_req_su);
    out.FF = 1;
    ...
    out.h_ex = h_ex_s;
    out.T_ex = T_is_ex;
    out.M = NaN; %(rho_su+CoolProp.PropsSI('
end
