function IC = InitialConditions_ORC_Ext_Npp_Nexp_3_Solub(fluid_wf, fluid_lub, fluid_htf, h_htf_su,  P_htf_su, m_dot_htf, fluid_ctf, h_ctf_su, P_ctf_su, m_dot_ctf, T_amb, N_exp, N_pp, C_oil, param)
...
T_ctf_su = CoolProp.PropsSI('T', 'H', h_ctf_su, 'P', P_ctf_su, fluid_ctf);
T_htf_su = PropsSI_ICP('T', 'H', h_htf_su, 'P', P_htf_su, fluid_htf);

P_pp_su_lb = max(CoolProp.PropsSI('P', 'Q', 0, 'T', T_ctf_su-5, fluid_wf), CoolProp.PropsSI('P_min', 'Q', 0, 'T', 273.15, fluid_wf));
...
for i_P_pp_su = 1: length(P_pp_su_guess0)
    for i_rp = 1: length(x_rp_guess0)
        for i_hev_ex = 1: length(x_h_ev_ex_guess0)
            ...
            try
                [guess, ~] = FCT_ORC_Ext_Npp_Nexp_3_Solub(x_test, lb_test, ub_test, fluid_wf, fluid_lub, fluid_htf, h_htf_su, P_htf_su, m_dot_htf, fluid_ctf, h_ctf_su, P_ctf_su, m_dot_ctf, T_amb, N_exp, N_pp, C_oil, param);
...
if isfield(param, 'x0')
    ...
    out_x0 = FCT_ORC_Ext_Npp_Nexp_3_Solub(max(min(param.x0(1:3), [P_pp_ex_ub P_pp_su_ub h_ev_ex_ub]), [P_pp_ex_lb P_pp_su_lb h_ev_ex_lb])./[P_pp_ex_ub P_pp_su_ub h_ev_ex_ub], [P_pp_ex_lb P_pp_su_lb h_ev_ex_lb], [P_pp_ex_ub P_pp_su_ub h_ev_ex_ub], fluid_wf, fluid_lub, fluid_htf, h_htf_su, P_htf_su, m_dot_htf, fluid_ctf, h_ctf_su, P_ctf_su, m_dot_ctf, T_amb, N_exp, N_pp, C_oil, param);  
    ...
    if param.restrict_bound || res_x0 < res_max
        delta_res = max(0, min(1,(res_x0-res_min)/(res_max-res_min)));      
        delta_P_pp_su = (1-delta_res)*0.5e5 + delta_res*2e5;
        delta_P_pp_ex = (1-delta_res)*0.1e5 + delta_res*2e5;
