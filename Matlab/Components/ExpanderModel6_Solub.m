function [out,TS] = ExpanderModel6_Solub(fluid, fluid_lub, C_oil, P_su, h_mix_su, N_exp, P_ex, T_amb, param)
...
x_req_su = CoolProp.PropsSI('Q','P',P_su,'H',h_req_su,fluid);
...
[rho_vap_su, rho_rl_su, rho_oil_su, rho_liq_su, K_su] = R245fa_POE_density(T_su, P_su, zeta_r_su, fluid, fluid_lub, param.fit_ratio_rho, Tbubble_min_su, Tsat_pure_su);
alpha_su = VoidFraction_SlipRatio(C_rv_su, rho_vap_su, rho_liq_su, 5);%(C_rv_su/rho_vap_su)/(C_oil/rho_oil_su + C_rl_su/rho_rl_su + C_rv_su/rho_vap_su);%C_rv_su; %
V_swept = param.V_s*(alpha_su + (1-alpha_su)*(C_rl_su/(C_rl_su+C_oil)));

%[alpha_su C_rl_su C_rv_su C_oil]
        ...
        case 'SemiEmp'
            ...
            while not(stop) && j <= length(x_T_guess)
                ...
                while not(stop) && k <= length(ff_guess)
                    ...
                    x0(1) = ff_guess(k)*param.V_s*N_exp/60*rho_req_su; %initial value for M_dot
                    ...
                    [x, res, flag] = fsolve(@(x)  Exp_SemiEmp_res(x, ub, fluid, P_su, h_req_su, N_exp, V_swept, param.r_v_in, P_ex, A_leak, param.d_su, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb, T_amb, param.C_loss, param.h_min, param.h_max, param), x0./ub, options);
            ...
            x = x.*ub;
            int = Exp_SemiEmp(x(1), x(2), fluid, P_su, h_req_su, N_exp, V_swept, param.r_v_in, P_ex, A_leak, param.d_su, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb, T_amb, param.C_loss, param.h_min, param.h_max, param);
            M_dot_r = x(1);
            M_dot = M_dot_r/(1-C_oil); 
            M_dot_oil = C_oil*M_dot;
            T_w = int.T_w;
            int.W_dot_r = int.W_dot;
            h_req_ex = int.h_ex;
            T_req_ex = CoolProp.PropsSI('T','P',P_ex,'H',h_req_ex,fluid);

            Q_dot_amb = int.Q_dot_amb;
            if int.resE < 1e-4 && int.resMlk <1e-4 && int.resgamma <1e-4 && h_req_ex > param.h_min && h_req_ex < param.h_max
                out.flag = flag;
            else
                out.flag = -1;
            end
            ...
            int.W_dot_oil = M_dot_oil*( (P_su-P_ex)/rho_oil_su) ;
            W_dot = int.W_dot_r + int.W_dot_oil;
            ...
            epsilon_is = W_dot/(M_dot_r*(h_req_su - h_req_ex_s) + int.W_dot_oil);
            %FF = M_dot/(param.V_s*N_exp/60*(alpha_su*rho_vap_su + (1-alpha_su)*rho_liq_su));
            FF = M_dot_r/(param.V_s*N_exp/60*rho_req_su);
        otherwise
...
if out.flag > 0;
    out.x_req_su = x_req_su;
    out.int = int;
    out.T_w = T_w;
    out.h_ex = h_ex;
    out.M_dot = M_dot;
    out.W_dot = W_dot;
    out.Q_dot_amb = NaN;%Q_dot_amb;
    out.epsilon_is = epsilon_is;
    out.FF = FF;
    out.T_ex = T_ex;
    out.M = 0;%(CoolProp.PropsSI('D','H',h_su,'P',P_su,fluid)+CoolProp.PropsSI('D','H',out.h_ex,'P',P_ex,fluid))/2*param.V;    
else    
    out.T_w = NaN;
    out.M_dot = N_exp/60*V_swept*rho_req_su;
    out.FF = 1;
    out.W_dot = out.M_dot*(h_req_su-h_req_ex_s);
    out.epsilon_is =1;
    out.Q_dot_amb = 0;
    out.h_ex = h_req_ex_s;
    out.T_ex = CoolProp.PropsSI('T','P',P_ex,'H',out.h_ex,fluid);
    out.M = (CoolProp.PropsSI('D','H',h_req_su,'P',P_su,fluid)+CoolProp.PropsSI('D','H',out.h_ex,'P',P_ex,fluid))/2*param.V;
end
