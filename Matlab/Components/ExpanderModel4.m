        ...
        case 'Piston_SemiEmp'
            ...
            while not(stop) && j <= length(x_T_guess)
                ...
                while not(stop) && k <= length(ff_guess)
                    ...
                    x0(4) = CoolProp.PropsSI('H','P',P_su,'H',h_su,fluid);% initial value for h_ex_3
                    
                    ub = 2*x0; % upper bound for fsolve
                    options = optimset('Display','iter');% 'TolX', 1e-8, 'TolFun', 1e-4);
                    [x, res, flag] = fsolve(@(x)  Exp_PistonSemiEmp_res(x, ub, fluid, P_su, h_su, N_exp, param.V_s, param.r_v_in, P_ex, param.A_leak0, param.d_su, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb, T_amb, param.C_loss, param.h_min, param.h_max, param), x0./ub, options);
                    if norm(res) < 1e-4 %flag > 0 
                        stop = 1;
                    end
                    k = k + 1;
            ...
            x = x.*ub;
            int = Exp_PistonSemiEmp(x(1), x(2), x(3), x(4), fluid, P_su, h_su, N_exp, param.V_s, param.r_v_in, P_ex, param.A_leak0, param.d_su, param.alpha, param.W_dot_loss_0, param.AU_su_n, param.M_dot_n, param.AU_ex_n, param.AU_amb, T_amb, param.C_loss, param.h_min, param.h_max, param);
            M_dot = x(1);
            W_dot = int.W_dot;
            h_ex = int.h_ex;
            epsilon_is = int.epsilon_is;
...
function res = Exp_PistonSemiEmp_res(x, ub, fluid, P_su, h_su, N_exp, V_s, r_v_in, P_ex, A_leak0, d_su, alpha, W_dot_loss_0, AU_su_n, M_dot_n, AU_ex_n, AU_amb, T_amb, C_loss, h_min, h_max, param)
x = x.*ub;
out = Exp_SemiEmp(x(1), x(2), x(3), fluid, P_su, h_su, N_exp, V_s, r_v_in, P_ex, A_leak0, d_su, alpha, W_dot_loss_0, AU_su_n, M_dot_n, AU_ex_n, AU_amb, T_amb, C_loss, h_min, h_max, param);
res = [out.resE ; out.resMlk; out.resP];
end

function out = Exp_PistonSemiEmp(M_dot, T_w, P_ex3, h_ex3, fluid, P_su, h_su, N_exp, V_s, rv_in_exp, rv_in_comp, P_ex, A_leak0, d_su, alpha, W_dot_loss_0, AU_su_n, M_dot_n, AU_ex_n, AU_amb, T_amb, C_loss, h_min, h_max, param)
if isnan(T_w)
    T_w =  CoolProp.PropsSI('T','P',P_su,'H',h_su,fluid);
end
M_dot = max(1e-5, M_dot);
...
out.gamma = max(1e-2,out.gamma);
P_crit = out.P_su2*(2/(out.gamma+1))^(out.gamma/(out.gamma-1));
out.resgamma = 0;
out.P_thr = max(P_ex_3,P_crit);
out.rho_thr = CoolProp.PropsSI('D','P',out.P_thr,'S',out.s_su2,fluid);
out.C_thr = sqrt(2*(out.h_su2 - CoolProp.PropsSI('H','P',out.P_thr,'S',out.s_su2,fluid)));
...
out.W_dot_3 = out.M_dot_comp*(out.h_ad2-out.h_ex3);
...
out.Q_dot_ex = max(0,out.epsilon_ex2*M_dot*out.cp_ex2*(T_w-out.T_ex2));
