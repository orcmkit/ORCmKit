function [out,TS] = ExpanderModel3(fluid, P_su, h_su, N_exp, P_ex, T_amb, param)
...
%         case 'SemiEmp'
%             
% cond.P_su = (P_su<=param.PolReg.P_su_max && P_su>=param.PolReg.P_su_min); 
% cond.T_amb = ( T_amb-273.15 <= param.PolReg.T_amb_max && T_amb-273.15 >= param.PolReg.T_amb_min); 
% cond.rp = ( P_su/P_ex <= param.PolReg.rp_max && P_su/P_ex >= param.PolReg.rp_min); 
% cond.h_su =  (h_su <= CoolProp.PropsSI('H','P',P_su,'T',param.PolReg.T_max,fluid) && h_su >= CoolProp.PropsSI('H','P',P_su,'Q',param.PolReg.x_min,fluid))
            if (P_su<=param.PolReg.P_su_max && P_su>=param.PolReg.P_su_min) && ( T_amb-273.15 <= param.PolReg.T_amb_max && T_amb-273.15 >= param.PolReg.T_amb_min) && ( P_su/P_ex <= param.PolReg.rp_max && P_su/P_ex >= param.PolReg.rp_min) && ( h_su <= CoolProp.PropsSI('H','P',P_su,'T',param.PolReg.T_max,fluid) && h_su >= CoolProp.PropsSI('H','P',P_su,'Q',param.PolReg.x_min,fluid)) && ( N_exp <= param.PolReg.N_max && N_exp>= param.PolReg.N_min)        
                Q_dot_amb = param.PolReg.f_Q_dot(T_amb/1e2, N_exp/1e3, P_su/1e5, P_ex/1e5, h_su/1e5)*1e3;
                W_dot = param.PolReg.f_W_dot(T_amb/1e2, N_exp/1e3, P_su/1e5, P_ex/1e5, h_su/1e5)*1e3;
                M_dot = param.PolReg.f_M_dot(T_amb/1e2, N_exp/1e3, P_su/1e5, P_ex/1e5, h_su/1e5)/1e3;
                h_ex = h_su -(W_dot+Q_dot_amb)/M_dot;
                FF = M_dot/(N_exp/60*param.V_s*rho_su);
                epsilon_is = W_dot/(M_dot*(h_su-h_ex_s));
                out.flag= 1;
            else
