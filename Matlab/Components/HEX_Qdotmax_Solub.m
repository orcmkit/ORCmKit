...
if strcmp(info.H.type,'H') && strcmp(info.C.type,'H') % CASE 1 : HOT FLUID AND COLD FLUID MIGHT EXPERIENCE A PHASE CHANGE
    ...
    h_c_su = in_c_su;
    if info.C.solub
        [T_c_su, ~, ~, ~, ~, ~, ~, ~, ~, ~] = HP_solubMixt(info.C.C_oil, P_c_su, h_c_su, fluid_c, info.C.fluid_lub);
    else
    ...
    if info.H.solub
        [T_h_su, ~, ~, ~, ~, ~, ~, ~, ~, ~] = HP_solubMixt(info.H.C_oil, P_h_su, h_h_su, fluid_h, info.H.fluid_lub);
    else
    ...
    if info.C.solub
        Tbubble_min_c = R245fa_POE_Tbubble(1-info.C.C_oil, P_c_su);
        Tsat_pure_c = CoolProp.PropsSI('T', 'P', P_c_su, 'Q', 0, fluid_c);
        [h_c_ex_extmax, ~, ~, ~, ~, ~, ~, ~] = TP_solubMixt(min(T_h_su, T_c_max), P_c_su, info.C.C_oil, fluid_c, info.C.fluid_lub, Tbubble_min_c, Tsat_pure_c);
    else
        if strcmp(fluid_c(1:3), 'ICP')
    ...
    if info.H.solub
        Tbubble_min_h = R245fa_POE_Tbubble(1-info.H.C_oil, P_h_su);
        Tsat_pure_h = CoolProp.PropsSI('T', 'P', P_h_su, 'Q', 0, fluid_h);
        [h_h_ex_extmax, ~, ~, ~, ~, ~, ~, ~] = TP_solubMixt(max(T_c_su, T_h_min), P_h_su, info.H.C_oil, fluid_h, info.H.fluid_lub, Tbubble_min_h, Tsat_pure_h);
    else
        if strcmp(fluid_h(1:3), 'ICP')
