function [out,TS] = HexModel_Solub_DP2(fluid_h, P_h_su, in_h_su, m_dot_h, fluid_c, P_c_su, in_c_su, m_dot_c, param)
...
if nargin == 0    % Define a demonstration case if HexModel.mat is not executed externally
    ...
    if strcmp(exemple, 'EV')
        ...
        fluid_h = 'ICP_PiroblocBasic_2';                                       	% Nature of the hot fluid           [-]
        m_dot_h = 0.999;                                          	% Mass flow rat of the hot fluid    [kg/s]
        ...
        param.H.C_oil = 0;
        param.H.DP_on = 0;
        h_ref = PropsSI_ICP('H','T', T_h_su,'P',P_h_su, fluid_h);      % Supply h or T of the cold fluid  	[J/kg pr K]
        ...
        param.C.fit_DTP_zeta = [];
        param.C.DP_on = 0;
        fluid_c = 'R245fa';                                                     % Nature of the cold fluid        	[-]
        m_dot_c = 0.35;                                                          % Mass flow rat of the cold fluid  	[kg/s]
        T_c_su = 50 + 273.15;
        P_c_su = 8e5;                                                          % Supply pressure of the cold fluid	[Pa]
        param.C.type = 'H';
        param.C.solub = 0;
        ...
        in_c_su = h_ref; %(1-param.C.C_oil)*h_ref + param.C.C_oil*h_lub;
        load('Ratio_RhoIdeal_R245Fa_POE.mat')
        ...
        param.H.correlation.type_1phase_l = 'S2P_evHTF_BPHEX';
        param.H.correlation.type_1phase_v = 'S2P_evHTF_BPHEX';
    ...
    if strcmp(exemple, 'REC')
        ...
        P_h_su =  6e5;                                              % Supply pressure of the hot fluid  [Pa]
        T_h_su = 90+273.15;
        param.H.DP_on = 0;
        param.H.f_dp = @(x) 14.14*1e5*(x)^1.892;
        param.H.dp = 0;
        param.H.type = 'H';
        param.H.solub = 0;
        param.H.fluid_lub = 'ICP_RL_32_3MAF';
        ...
        fluid_c = 'R245fa';                                                     % Nature of the cold fluid        	[-]
        m_dot_c = 0.0105;                                                          % Mass flow rat of the cold fluid  	[kg/s]
        T_c_su = 30 + 273.15;
        ...
        param.H.correlation.type_void_fraction = 'Hughmark';
        param.C.correlation.type_void_fraction = 'Hughmark';
...
[h_h_l_in, h_h_v_in, T_h_su] = supply_conditions_HEX_Solub(in_h_su, P_h_su, fluid_h, param.H);

% Evaluation of the cold fluid (CF) supply conditions
[h_c_l_in, h_c_v_in, T_c_su] = supply_conditions_HEX_Solub(in_c_su, P_c_su, fluid_c, param.C);
...
if T_h_su<T_c_su
    ...
    [fluid_h_int, P_h_su_int, in_h_su_int, m_dot_h_int, T_h_su_int, h_h_l_in_int, h_h_v_in_int, H_int] = deal(fluid_h, P_h_su, in_h_su, m_dot_h, T_h_su, h_h_l_in, h_h_v_in, param.H);
    [fluid_h, P_h_su, in_h_su, m_dot_h, T_h_su, h_h_l_in, h_h_v_in, param2.H] = deal(fluid_c, P_c_su, in_c_su, m_dot_c, T_c_su, h_c_l_in, h_c_v_in,  param.C);
    [fluid_c, P_c_su, in_c_su, m_dot_c, T_c_su, h_c_l_in, h_c_v_in, param2.C] = deal(fluid_h_int, P_h_su_int, in_h_su_int, m_dot_h_int, T_h_su_int, h_h_l_in_int, h_h_v_in_int, H_int);
    clear param; param = param2;
    flag_reverse = 1;
end

if param.H.DP_on
    DP_h = max(0,param.H.f_dp(m_dot_h));   
    if strcmp(fluid_h(1:3), 'ICP')
        T_h_ex_isenthalpy = T_h_su;
    else
        if param.H.solub
            T_sat_pure = CoolProp.PropsSI('T', 'P', P_h_su-DP_h, 'Q', 0, fluid_h);
            T_min_bubble = R245fa_POE_Tbubble(1-param.H.C_oil, P_h_su-DP_h, T_sat_pure);
            [T_h_ex_isenthalpy, ~, ~, ~, ~, ~, ~, ~] = HP_solubMixt(param.H.C_oil, P_h_su-DP_h, in_h_su, fluid_h, param.H.fluid_lub, T_min_bubble, T_sat_pure, param.H.fit_DTP_zeta);
        else
            T_h_ex_isenthalpy = CoolProp.PropsSI('T','H', in_h_su, 'P', P_h_su-DP_h, fluid_h);
        end
    end   
    if T_h_ex_isenthalpy - T_c_su < 0
        DP_h = 0;
        param.H.DP_on = 0;
    end
else
...
if param.C.DP_on
    DP_c = max(0,param.C.f_dp(m_dot_c));
else
    DP_c = 0;
end
if (T_h_su-T_c_su)>1e-2  && m_dot_h  > 0 && m_dot_c > 0; % Check if the operating conditions permit a viable heat transfer
    ...
    %    out_max = HEX_profile_Solub_DP2(fluid_h, m_dot_h, P_h_su, in_h_su, fluid_c, m_dot_c, P_c_su, in_c_su, Q_dot_max, param, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c);
    %    pinch_Qdot_max = out_max.pinch;
    %else
        [Q_dot_max, pinch_Qdot_max] = HEX_Qdotmax_Solub_DP2(fluid_h, m_dot_h, P_h_su, in_h_su, T_h_su, fluid_c, m_dot_c, P_c_su, in_c_su, T_c_su, param, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c); % compute maximal heat transfer
    %end
    lb = 0;
    ub = Q_dot_max;
    f = @(Q_dot) HEX_hConvCorSolub_res(fluid_h, m_dot_h, P_h_su, in_h_su, fluid_c, m_dot_c, P_c_su, in_c_su,  Q_dot, param, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c);
    %figure
    ...
    if f(ub) > 0
        ...
        Q_dot_eff = zeroBrent ( lb, ub, 1e-16, 1e-14, f, 1e-10 ); % Solver driving residuals of HEX_hConvVar_res to zero
    end
else
    Q_dot_eff = 1;
    Q_dot_max = 1;
...
out = HEX_hConvCorSolub(fluid_h, m_dot_h, P_h_su, in_h_su, fluid_c, m_dot_c, P_c_su, in_c_su, real(Q_dot_eff), param, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c);
out.H.fluid = fluid_h;
out.C.fluid = fluid_c;
...
out.H.P_ex = out.H.P_vec(1);
out.C.P_ex = out.C.P_vec(end);
...
out.H.hConv_liq_mean = NaN;%mean(out.H.hConv_vec(strcmp(out.H.type_zone, 'liq')));
out.H.hConv_tp_mean = NaN;%mean(out.H.hConv_vec(strcmp(out.H.type_zone, 'tp') | strcmp(out.H.type_zone, 'tp_dryout')));
out.H.hConv_vap_mean = NaN;%mean(out.H.hConv_vec(strcmp(out.H.type_zone, 'vap') | strcmp(out.H.type_zone, 'vap_wet')));
out.C.hConv_liq_mean = NaN;%mean(out.C.hConv_vec(strcmp(out.C.type_zone, 'liq')));
out.C.hConv_tp_mean = NaN;%mean(out.C.hConv_vec(strcmp(out.C.type_zone, 'tp') | strcmp(out.C.type_zone, 'tp_dryout')));
out.C.hConv_vap_mean = NaN;%mean(out.C.hConv_vec(strcmp(out.C.type_zone, 'vap') | strcmp(out.C.type_zone, 'vap_wet')));
