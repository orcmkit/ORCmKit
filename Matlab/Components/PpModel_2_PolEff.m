function [out,TS] = PpModel_2_PolEff(P_su, h_su, P_ex, fluid, M_dot, param)
...
% rdickes @ulg.ac.be
%
% "PpModel_2_PolEff.m" is a single matlab function implementing a constant-efficiency model
% of volumetric pump (see the Documentation/PpModel_2_PolEff_MatlabDoc). Unlike "PpModel_PolEff.m" 
% which imposes N_pp and deduces the mass flow rate, "PpModel_2_PolEff.m" imposes the mass flow rate
...
%           param.f_epsilon_is, function handle related to eps_is   [-]
%           param.f_epsilon_vol, function handle related to eps_vol [-]
...
if P_su < P_ex && M_dot > 0
    V_s = param.V_s;
    epsilon_is = param.f_epsilon_is((M_dot/param.M_dot_nom),(P_ex/P_su));
    epsilon_vol = param.f_epsilon_vol((M_dot/param.M_dot_nom),(P_ex/P_su));
