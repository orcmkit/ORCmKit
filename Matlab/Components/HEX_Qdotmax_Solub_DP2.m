function [Q_dot_max, pinch_min] = HEX_Qdotmax_Solub_DP2(fluid_h, m_dot_h, P_h_su, in_h_su, T_h_su, fluid_c, m_dot_c, P_c_su, in_c_su, T_c_su, info, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c)
%% CODE DESCRIPTION
...
if strcmp(info.H.type,'H') && strcmp(info.C.type,'H') % CASE 1 : HOT FLUID AND COLD FLUID MIGHT EXPERIENCE A PHASE CHANGE
    ...
    lb = 0;
    f = @(x) pinch0_Solub(fluid_h, m_dot_h, P_h_su, in_h_su, fluid_c, m_dot_c, P_c_su, in_c_su, x, info, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c);
    [Q_dot_max, pinch_min ]= zeroBrent ( lb, ub, 1e-12, 1e-8, f, 1e-5);
end

end

function err = pinch0_Solub(fluid_h, m_dot_h, P_h_su, in_h_su, fluid_c, m_dot_c, P_c_su, in_c_su, Q_dot, info, h_h_l_in, h_h_v_in, h_c_l_in, h_c_v_in, DP_h, DP_c)
