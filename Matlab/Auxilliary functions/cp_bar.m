function [cpbar,Q_4,x,e_min,e] = cp_bar(m,n,f,T_p1,T_p2)
...
MM_CO = PropsSI('M','T',298,'P',1e5,'CO');
MM_N2 = PropsSI('M','T',298,'P',1e5,'N2');
MM_O2 = PropsSI('M','T',298,'P',1e5,'O2');
MM_CO2 = PropsSI('M','T',298,'P',1e5,'CO2');
MM_H2O = PropsSI('M','T',298,'P',1e5,'water');
...
if (f>0) %produits de combustion et pas de l'air pur
    ...
    if f>f_st %défaut d'air
        ...
        h_CO2 = (PropsSI('H','P',1e5,'T',T_p1+273.15,'CO2')-PropsSI('H','P',1e5,'T',T_p2+273.15,'CO2'))*MM_CO2;
        h_CO = (PropsSI('H','P',1e5,'T',T_p1+273.15,'CO')-PropsSI('H','P',1e5,'T',T_p2+273.15,'CO'))*MM_CO;
        a_H20 = -1.34618e7;
        b_H20 = 1785.27;
        c_H20 = 0.354129;
        d_H20 = -0.00000299621e-9;
        h_H2O = ((a_H20 +b_H20*T_p1 + c_H20*T_p1^2 + d_H20*T_p1^3) - (a_H20 +b_H20*T_p2 + c_H20*T_p2^2 + d_H20*T_p2^3))*MM_H2O;
        h_N2 = (PropsSI('H','P',1e5,'T',T_p1+273.15,'N2')-PropsSI('H','P',1e5,'T',T_p2+273.15,'N2'))*MM_N2;
        cpbar = ((x_CO2*h_CO2+x_CO*h_CO+x_H2O*h_H2O+x_N2*h_N2)/mmprod)/(T_p1-T_p2);
        %Pertes par imbrûlés par kmol de combustible
        T_ref=25;
        LHV_CO_mol = PropsSI('H','P',1e5,'T',T_ref+273.15,'CO')*MM_CO+0.5*PropsSI('H','P',1e5,'T',T_ref+273.15,'O2')*MM_O2-PropsSI('H','P',1e5,'T',T_ref+273.15,'CO2')*MM_CO2;
        ...
        hp_1 = (PropsSI('H','P',1e5,'T',T_p1+273.15,'CO2')-PropsSI('H','P',1e5,'T',T_p2+273.15,'CO2'))*MM_CO2;
        a_H20 = -1.34618e7;
        b_H20 = 1785.27;
        c_H20 = 0.354129;
        d_H20 = -0.00000299621e-9;
        hp_2 = ((a_H20 +b_H20*T_p1 + c_H20*T_p1^2 + d_H20*T_p1^3) - (a_H20 +b_H20*T_p2 + c_H20*T_p2^2 + d_H20*T_p2^3))*MM_H2O;
        hp_3 = (PropsSI('H','P',1e5,'T',T_p1+273.15,'O2')-PropsSI('H','P',1e5,'T',T_p2+273.15,'O2'))*MM_O2;
        hp_4 = (PropsSI('H','P',1e5,'T',T_p1+273.15,'N2')-PropsSI('H','P',1e5,'T',T_p2+273.15,'N2'))*MM_N2;
