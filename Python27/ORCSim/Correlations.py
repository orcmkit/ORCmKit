...
def T_hp(FluidName, h, p, T_guess):
    #print 'T_hp p:', p
    if FluidName =='POE':
        f = lambda T: h_l(FluidName, T[0], p)-h
...
def TrhoPhase_ph(Ref,p,h,Tbubble,Tdew,rhosatL=None,rhosatV=None):
    ...
    UseSaturationLUT(1)
    R_u=8.31447
    #h=h/1000  #convert J/kg to kJ/kg since CoolProp uses kJ/kg
    #print 'p [kPa]:',p
    #TODO: Modified for Incompressible as hot fluid
    if IsFluidType(Ref,'Brine'):
        ...
        T=T_hp(Ref,h,p,Tguess)
        rho=PropsSI('D','T',T,'P',p,Ref)
        return T,rho,'Subcooled'
    elif Ref =='INCOMP::T66':
        #It is subcooled 
        Tguess=PropsSI('Tmax','T',0,'P',0, Ref)-10
        T=T_hp(Ref,h,p*1000,Tguess)
        rho=PropsSI('D','T',T,'P',p*1000,Ref)
        return T,rho,'Subcooled'
    elif Ref =='POE':
        Tmax = 120 + 273.15  # 120C max temp validity viscosity correlation
        Tguess = Tmax - 10 
        T=T_hp(Ref,h,p,Tguess)
        rho = rho_l(Ref,T)
        return T,rho,'Subcooled'
    ...
    if p*1000>PropsSI(Ref,'Pcrit'):
        raise ValueError('Supercritical fluid cannot be handled by the model')
    ...
    #It is not supercritical
        ...
        if h>hsatV:
            ...
            #TODO: originally rho=PropsSI('D','T',T,'P',p*1000,Ref)
            
            rho=PropsSI('D','H',h,'P',p*1000,Ref)
            return T,rho,'Subcooled'
        else:
            #It's two-phase
            x=(h-hsatL)/(1.0e-6+hsatV-hsatL)
            v=x*vsatV+(1-x)*vsatL
...
def TwoPhaseDensity(Ref,xmin,xmax,Tdew,Tbubble,slipModel='Zivi'):
    ...
    if xmin==xmax:
        alpha_average=1/(1+C*(1-xmin)/(xmin+1.0e-6))
    else:    
        if xmin>=1.0:
            alpha_average=1.0
        elif xmax<=0.0:
    ...
    def f(x,rhoL,rhoV):
        if x==0.0:
            ...
            if slipModel=='Zivi':
                ...
                raise ValueError("slipModel must be either 'Zivi' or 'Homogeneous'")
            alpha=1/(1+S*rhoL/rhoV*(1-x)/x)
            return x**2/rhoV/alpha+(1-x)**2/rhoL/(1-alpha)
    return G**2*(f(x_max,rhosatL,rhosatV)-f(x_min,rhosatL,rhosatV))
...
def ShahEvaporation_Average(x_min,x_max,Ref,G,D,p,q_flux,Tbubble,Tdew):
    ...
    * Tdew : Dewpoint temperature of refrigerant [K]
    ...
    h_fg = (PropsSI('H', 'T', Tdew, 'Q', 1, Ref) - Props('H', 'T', Tbubble, 'Q', 0, Ref))*1000 #[J/kg]
...
def f_h_1phase_Annulus(mdot, OD, ID, T, p, Fluid, Phase='Single'):
    ...
    if Phase =="SatVap":
        ...
        cp = PropsSI('C', 'T', T, 'Q', 1, Fluid)#*1000. #J/kg-K
        k = PropsSI('L', 'T', T, 'Q', 1, Fluid)#*1000. #W/m-K
        rho = PropsSI('D', 'T', T, 'Q', 1, Fluid) #kg/m^3
    else:
        mu = PropsSI('V', 'T', T, 'P', p*1000+100, Fluid)  #kg/m-s
        cp = PropsSI('C', 'T', T, 'P', p*1000+100, Fluid)#*1000. #J/kg-K
        k = PropsSI('L', 'T', T, 'P', p*1000+100, Fluid)#*1000. #W/m-K
        rho = PropsSI('D', 'T', T, 'P', p*1000+100, Fluid) #kg/m^3
...
def f_h_1phase_Channel(mdot,W,H,T,p,Fluid,Phase):
    ...
    if Phase=="SatVap":
        ...
        mu = PropsSI('V', 'T', T, 'P', p*1000, Fluid); ##kg/m-s
        cp = PropsSI('C', 'T', T, 'P', p*1000, Fluid)#*1000. #J/kg-K
        k = PropsSI('L', 'T', T, 'P', p*1000, Fluid)#*1000. #W/m-K
        rho = PropsSI('D', 'T', T, 'P', p*1000, Fluid) #kg/m^3
...
def PHE_1phase_hdP(Inputs,JustGeo=False):
    ...
    if JustGeo==True:
        ...
        #"Glycol" properties
        if Ref == 'POE':
            rho_g=rho_l(Ref,T) #kg/m3
            eta_g=mu_l(Ref,T) #Pa-s
            cp_g=cp_l(Ref,T) #J/kg-K
            k_g=k_l(Ref,T) #W/m-K
        elif Ref == 'ACD100FY':
            rho_g=rho_l(Ref,T) #kg/m3
            eta_g=mu_l(Ref,T) #Pa-s
            ...
            rho_g=PropsSI('D','T',T,'P',p*1000+100,Ref)
            eta_g=PropsSI('V','T',T,'P',p*1000+100,Ref)
            cp_g=PropsSI('C','T',T,'P',p*1000+100,Ref)#*1000
            k_g=PropsSI('L','T',T,'P',p*1000+100,Ref)#*1000
...
def Cooper_PoolBoiling(pstar,Rp,q,M):
    ...
    #return 55*pstar**(0.12-0.0087*log(Rp))*(-0.4343*log(pstar))**(-0.55)*q**(0.67)*M**(-0.5)
...
def KandlikarPHE(Ref,xmean,G,D,q,Tbubble,Tdew):
    ...
    h_LG=PropsSI('H','T',Tdew,'Q',1,Ref)-PropsSI('H','T',Tbubble,'Q',0,Ref)
